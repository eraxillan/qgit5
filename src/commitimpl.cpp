...
CommitImpl::CommitImpl( Git* g, bool amend ) : git( g )
{
    ...
    if( amend )
    {
        ...
        connect( pushButtonOk, SIGNAL( clicked() ), this, SLOT( pushButtonAmend_clicked() ) );
    }
    else
    {
        if( git->isStGITStack() )
        ...
        connect( pushButtonOk, SIGNAL( clicked() ), this, SLOT( pushButtonCommit_clicked() ) );
    }
    connect( treeWidgetFiles, SIGNAL( customContextMenuRequested( const QPoint& ) ), this,
             SLOT( contextMenuPopup( const QPoint& ) ) );
    connect( textEditMsg, SIGNAL( cursorPositionChanged() ), this,
             SLOT( textEditMsg_cursorPositionChanged() ) );
}

void
CommitImpl::closeEvent( QCloseEvent* )
{
    QVector< QSplitter* > v( 1, splitter );
...
void
CommitImpl::contextMenuPopup( const QPoint& pos )
{
...
void
CommitImpl::unCheckAll()
{
    checkUncheck( false );
}

void
CommitImpl::checkUncheck( bool checkAll )
{
...
bool
CommitImpl::getFiles( SList selFiles )
{
...
void
CommitImpl::warnNoFiles()
{
    QMessageBox::warning( this, "Commit changes - QGit",
                          "Sorry, no files are selected for updating.", QMessageBox::Ok,
                          QMessageBox::NoButton );
}

bool
CommitImpl::checkFiles( SList selFiles )
{
...
bool
CommitImpl::checkMsg( QString& msg )
{
    ...
    {
        QMessageBox::warning( this, "Commit changes - QGit",
                              "Sorry, I don't want an empty message.", QMessageBox::Ok,
                              QMessageBox::NoButton );
        return false;
    }
...
bool
CommitImpl::checkPatchName( QString& patchName )
{
    bool ok;
    ...
    if( patchName != tmp.remove( ' ' ) )
        QMessageBox::warning( this, "Create new patch - QGit",
                              "Sorry, control "
...
bool
CommitImpl::checkConfirm( SCRef msg, SCRef patchName, SCList selFiles, bool amend )
{
    ...
    // FIXME: i18n-ugly
    QString whatToDo =
        amend ? ( git->isStGITStack() ? "refresh top patch with" : "amend last commit with" )
...
void
CommitImpl::pushButtonSettings_clicked()
{
...
void
CommitImpl::pushButtonCancel_clicked()
{
    close();
}

void
CommitImpl::pushButtonCommit_clicked()
{
...
void
CommitImpl::pushButtonAmend_clicked()
{
    QStringList selFiles;  // Retrieve selected files
    getFiles( selFiles );
...
void
CommitImpl::pushButtonUpdateCache_clicked()
{
...
void
CommitImpl::textEditMsg_cursorPositionChanged()
{
    int col_pos, line_pos;
    computePosition( col_pos, line_pos );
    QString lineNumber = QString( "Line: %1 Col: %2" ).arg( line_pos + 1 ).arg( col_pos + 1 );
    textLabelLineCol->setText( lineNumber );
}

void
CommitImpl::computePosition( int& col_pos, int& line_pos )
{
