...
ExceptionManager::Exception::Exception( int ex, bool v )
    : excpId( ex ), verbose( v ), isRaised( false )
{
}
...
void
ExceptionManager::init( int* excp, const QString& desc )
{
    *excp = excpId++;
...
bool
ExceptionManager::isMatch( int value, int excp, const QString& context )
{
    bool match = ( value == excp );
    if( match )
    {
        QString info( "Caught exception \'" + descriptions[ excp ] + "\' while in " + context );
...
void
ExceptionManager::add( int excpId, bool verbose )
{
    //
...
void
ExceptionManager::remove( int excpId )
{
    //
...
ExceptionManager::SetIt
ExceptionManager::findExcp( ThrowableSet& ts, const SetIt& startIt, int excpId )
{
    SetIt it( startIt );
    for( ; it != ts.end(); ++it )
    {
...
void
ExceptionManager::setRaisedFlag( ThrowableSet& ts, int excpId )
{
...
void
ExceptionManager::raise( int excpId )
{
...
int
ExceptionManager::saveThrowableSet()
{
    //
...
void
ExceptionManager::restoreThrowableSet( int regionId )
{
...
bool
ExceptionManager::isPending( int excpId )
{
    //
