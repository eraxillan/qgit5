...
#define SHOW_MSG( x )                                                                              \
    QApplication::postEvent( parent(), new MessageEvent( x ) );                                    \
...
bool
Git::TreeEntry::operator<( const TreeEntry& te ) const
{
...
Git::checkEnvironment()
{
    ...
    if( run( "git --version", &version ) )
    {
        ...
        if( version < GIT_VERSION )
        {
            ...
            const QString cmd( "Current git version is " + version + " but is required " +
                               GIT_VERSION + " or better" );

            const QString errorDesc(
                "Your installed git is too old."
...
Git::userInfo( SList info )
{
    ...
    QString user(
        env.section( "GIT_AUTHOR_NAME", 1 ).section( ",", 0, 0 ).section( "=", 1 ).trimmed() );
    QString email(
        env.section( "GIT_AUTHOR_EMAIL", 1 ).section( ",", 0, 0 ).section( "=", 1 ).trimmed() );
...
//
// TODO: investigate if there is a better way of getting this (from git e.g.)
//
bool
Git::isBinaryFile( SCRef file )
{
    static const char* binaryFileExtensions[] = {"bmp", "gif", "jpeg", "jpg", "png", "svg", "tiff",
                                                 "pcx", "xcf", "xpm",  "bz",  "bz2", "rar", "tar",
                                                 "z",   "gz",  "tgz",  "zip", 0};
...
bool
Git::isThrowOnStopRaised( int excpId, SCRef curContext )
{
    return EM_MATCH( excpId, exGitStopped, curContext );
}

void
Git::setTextCodec( QTextCodec* tc )
{
#if QT_VERSION < 0x050000
    ...
    //
    // FIXME: workaround Qt issue of mime name different from
...
// TODO: utility function; can go elsewhere
const QString
Git::quote( SCRef nm )
{
...
uint
Git::checkRef( const ShaString& sha, uint mask ) const
{
...
uint
Git::checkRef( SCRef sha, uint mask ) const
{
...
const QStringList
Git::getRefName( SCRef sha, RefType type, QString* curBranch ) const
{
...
const QString
Git::getRefSha( SCRef refName, RefType type, bool askGit )
{
...
void
Git::appendNamesWithId( QStringList& names, SCRef sha, SCList data, bool onlyLoaded )
{
...
Git::getTagMsg( SCRef sha )
{
    ...
    QRegExp pgp( "-----BEGIN PGP SIGNATURE*END PGP SIGNATURE-----", Qt::CaseSensitive,
                 QRegExp::Wildcard );
...
void
Git::addExtraFileInfo( QString* rowName, SCRef sha, SCRef diffToSha, bool allMergeFiles )
{
...
void
Git::removeExtraFileInfo( QString* rowName )
{
...
void
Git::formatPatchFileHeader( QString* rowName, SCRef sha, SCRef diffToSha, bool combined,
                            bool allMergeFiles )
{
    if( combined )
    {
...
//
Annotate*
Git::startAnnotate( FileHistory* fh, QObject* guiObj )
{
...
void
Git::cancelAnnotate( Annotate* ann )
{
    if( ann )
        ann->deleteWhenDone();
}
...
Git::lookupAnnotation( Annotate* ann, SCRef sha )
{
    return ( ann ? ann->lookupAnnotation( sha ) : NULL );
}

void
Git::cancelDataLoading( const FileHistory* fh )
{
    //
...
const Rev*
Git::revLookup( SCRef sha, const FileHistory* fh ) const
{
    return revLookup( toTempSha( sha ), fh );
}

const Rev*
Git::revLookup( const ShaString& sha, const FileHistory* fh ) const
{
...
bool
Git::run( SCRef runCmd, QString* runOutput, QObject* receiver, SCRef buf )
{
    QByteArray ba;
...
bool
Git::run( QByteArray* runOutput, SCRef runCmd, QObject* receiver, SCRef buf )
{
...
MyProcess*
Git::runAsync( SCRef runCmd, QObject* receiver, SCRef buf )
{
...
MyProcess*
Git::runAsScript( SCRef runCmd, QObject* receiver, SCRef buf )
{
...
int
Git::findFileIndex( const RevFile& rf, SCRef name )
{
    if( name.isEmpty() )
        return -1;
...
const QString
Git::getLaneParent( SCRef fromSHA, int laneNum )
{
...
MyProcess*
Git::getDiff( SCRef sha, QObject* receiver, SCRef diffToSha, bool combined )
{
    if( sha.isEmpty() )
        return NULL;
...
const QString
Git::getFileSha( SCRef file, SCRef revSha )
{
    if( revSha == ZERO_SHA )
    {
...
MyProcess*
Git::getFile( SCRef fileSha, QObject* receiver, QByteArray* result, SCRef fileName )
{
    QString runCmd;
    //
...
MyProcess*
Git::getHighlightedFile( SCRef fileSha, QObject* receiver, QString* result, SCRef fileName )
{
...
bool
Git::saveFile( SCRef fileSha, SCRef fileName, SCRef path )
{
    QByteArray fileData;
...
bool
Git::getTree( SCRef treeSha, TreeInfo& ti, bool isWorkingDir, SCRef path )
{
    QStringList deleted;
    if( isWorkingDir )
    {
...
void
Git::getWorkDirFiles( SList files, SList dirs, RevFile::StatusFlag status )
{
    files.clear();
    dirs.clear();
...
bool
Git::isParentOf( SCRef par, SCRef child )
{
...
bool
Git::isSameFiles( SCRef tree1Sha, SCRef tree2Sha )
{
    //
...
const QStringList
Git::getDescendantBranches( SCRef sha, bool shaOnly )
{
    QStringList tl;
    const Rev* r = revLookup( sha );
...
const QStringList
Git::getNearTags( bool goDown, SCRef sha )
{
    QStringList tl;
    const Rev* r = revLookup( sha );
    if( !r )
...
// TODO: utility function; can go elsewhere
const QString
Git::colorMatch( SCRef txt, QRegExp& regExp )
{
    QString text;
#if QT_VERSION < 0x050000
    text = Qt::escape( txt );
...
// TODO: utility function; can go elsewhere
const QString
Git::formatList( SCList sl, SCRef name, bool inOneLine )
{
    if( sl.isEmpty() )
        return QString();
...
const QString
Git::getDesc( SCRef sha, QRegExp& shortLogRE, QRegExp& longLogRE, bool showHeader, FileHistory* fh )
{
    if( sha.isEmpty() )
        return "";
    ...
    else
    {
        ...
        ts << "<html><head><style type=\"text/css\">"
              "tr.head { background-color: "
           << QPalette().color( QPalette::Mid ).name() << " }\n"
                                                          "td.h { font-weight: bold; }\n"
                                                          "table { background-color: "
           << QPalette().color( QPalette::Button ).name()
           << "; }\n"
...
const RevFile*
Git::insertNewFiles( SCRef sha, SCRef data )
{
    //
...
bool
Git::runDiffTreeWithRenameDetection( SCRef runCmd, QString* runOutput )
{
    //
...
const RevFile*
Git::getAllMergeFiles( const Rev* r )
{
...
const RevFile*
Git::getFiles( SCRef sha, SCRef diffToSha, bool allFiles, SCRef path )
{
    const Rev* r = revLookup( sha );
    if( !r )
        return NULL;
...
bool
Git::startFileHistory( SCRef sha, SCRef startingFileName, FileHistory* fh )
{
...
const QString
Git::getNewestFileName( SCList branches, SCRef fileName )
{
...
void
Git::getFileFilter( SCRef path, ShaSet& shaSet ) const
{
    shaSet.clear();
...
bool
Git::getPatchFilter( SCRef exp, bool isRegExp, ShaSet& shaSet )
{
    shaSet.clear();
    QString buf;
    FOREACH( ShaVect, it, revData->revOrder )
...
bool
Git::resetCommits( int parentDepth )
{
...
bool
Git::applyPatchFile( SCRef patchPath, bool fold, bool isDragDrop )
{
    if( isStGIT )
    {
        if( fold )
...
bool
Git::formatPatch( SCList shaList, SCRef dirPath, SCRef remoteDir )
{
...
const QStringList
Git::getOtherFiles( SCList selFiles, bool onlyInIndex )
{
...
bool
Git::updateIndex( SCList selFiles )
{
    const RevFile* files = getFiles( ZERO_SHA );  // files != NULL
...
bool
Git::commitFiles( SCList selFiles, SCRef msg, bool amend )
{
...
bool
Git::mkPatchFromWorkDir( SCRef msg, SCRef patchFile, SCList files )
{
    //
...
bool
Git::stgCommit( SCList selFiles, SCRef msg, SCRef patchName, bool fold )
{
    //
...
bool
Git::makeBranch( SCRef sha, SCRef branchName )
{
    return run( "git branch " + branchName + " " + sha );
}

bool
Git::makeTag( SCRef sha, SCRef tagName, SCRef msg )
{
    if( msg.isEmpty() )
...
const QStringList
Git::getArgs( bool* quit, bool repoChanged )
{
    QString args;
    if( startup )
    {
        ...
        {
            //
            // In arguments with spaces double quotes are stripped by Qt, so re-add them
            //
...
bool
Git::getGitDBDir( SCRef wd, QString& gd, bool& changed )
{
    //
...
bool
Git::getBaseDir( SCRef wd, QString& bd, bool& changed )
{
    //
    // We could run from a subdirectory, so we need to get correct directories
    //
    // We use --show-cdup and not --git-dir for this, in order to take into account configurations
    // in which .git is indeed a "symlink", a text file containing the path of the actual .git
    // database dir
    // In that particular case, the parent directory of the one given by --git-dir is *not*
    // necessarily the base directory of the repository
    //
...
Git::Reference*
Git::lookupOrAddReference( const ShaString& sha )
{
...
Git::Reference*
Git::lookupReference( const ShaString& sha )
{
    RefMap::iterator it( refsShaMap.find( sha ) );
    if( it == refsShaMap.end() )
        return 0;
...
void
Git::parseStGitPatches( SCList patchNames, SCList patchShas )
{
    ...
    FOREACH_SL( it, pl )
    {
        ...
        {
            dbp(
                "ASSERT in Git::parseStGitPatches(), patch %1 "
                "not found in references list.",
                patchName );
            continue;
        }
...
Rev*
Git::fakeRevData( SCRef sha, SCList parents, SCRef author, SCRef date, SCRef log, SCRef longLog,
...
const Rev*
Git::fakeWorkDirRev( SCRef parent, SCRef log, SCRef longLog, int idx, FileHistory* fh )
{
    QString patch;
    if( !isMainHistory( fh ) )
...
const RevFile*
Git::fakeWorkDirRevFile( const WorkingDirInfo& wd )
{
    FileNamesLoader fl;
    RevFile* rf = new RevFile();
...
void
Git::parseDiffFormatLine( RevFile& rf, SCRef line, int parNum, FileNamesLoader& fl )
{
    if( line[ 1 ] == ':' )
    {
        //
...
// TODO: can go in RevFile
void
Git::setStatus( RevFile& rf, SCRef rowSt )
{
    ...
    default:
        dbp(
            "ASSERT in Git::setStatus, unknown status <%1>. "
            "'MODIFIED' will be used instead.",
            rowSt );
        rf.status.append( RevFile::MODIFIED );
        break;
    }
...
void
Git::setExtStatus( RevFile& rf, SCRef rowSt, int parNum, FileNamesLoader& fl )
{
...
// TODO: utility function; can go elsewhere
void
Git::parseDiffFormat( RevFile& rf, SCRef buf, FileNamesLoader& fl )
{
...
bool
Git::startParseProc( SCList initCmd, FileHistory* fh, SCRef buf )
{
    DataLoader* dl = new DataLoader( this, fh );  // NOTE: auto-deleted when done

    connect( this, SIGNAL( cancelLoading( const FileHistory* ) ), dl,
             SLOT( on_cancel( const FileHistory* ) ) );

    connect( dl, SIGNAL( newDataReady( const FileHistory* ) ), this,
             SLOT( on_newDataReady( const FileHistory* ) ) );

    connect( dl, SIGNAL( loaded( FileHistory*, ulong, int, bool, const QString&, const QString& ) ),
             this,
             SLOT( on_loaded( FileHistory*, ulong, int, bool, const QString&, const QString& ) ) );

    return dl->start( initCmd, workDir, buf );
}

bool
Git::startRevList( SCList args, FileHistory* fh )
{
    QString baseCmd(
        "git log --topo-order --no-color "
    ...
    {
        // initCmd << QString("--early-output"); // NOTE: currently disabled
    }
...
Git::startUnappliedList()
{
    ...
    //
    // NOTE: WARNING: with this command 'git log' could send spurious
    // revs so we need some filter out logic during loading
    //
    QString cmd(
        "git log --no-color --parents -z "
...
bool
Git::init( SCRef wd, bool askForRange, const QStringList* passedArgs, bool overwriteArgs,
           bool* quit )
{
    //
    ...
    *quit = false;
    ...
    try
    {
        ...
        if( !passedArgs )
        {
            ...
            {
                loadArguments.args =
                    getArgs( quit, repoChanged );  // Must be called with refs loaded
                if( *quit )
                {
                    setThrowOnStop( false );
                    return false;
...
void
Git::on_newDataReady( const FileHistory* fh )
{
    emit newRevsAdded( fh, fh->revOrder );
}

void
Git::on_loaded( FileHistory* fh, ulong byteSize, int loadTime, bool normalExit, SCRef cmd,
                SCRef errorDesc )
{
...
bool
Git::tryFollowRenames( FileHistory* fh )
{
    if( isMainHistory( fh ) )
        return false;
...
bool
Git::populateRenamedPatches( SCRef renamedSha, SCList newNames, FileHistory* fh,
                             QStringList* oldNames, bool backTrack )
{
    ...
    if( prevFileSha == lastFileSha )  // just renamed
        runOutput.clear();
    else if( !run( "git diff --no-ext-diff -r --full-index " + prevFileSha + " " + lastFileSha,
                   &runOutput ) )
        return false;
    ...
    if( fh )
    {
        QString tmp( !runOutput.isEmpty() ? runOutput
...
bool
Git::filterEarlyOutputRev( FileHistory* fh, Rev* rev )
{
...
int
Git::addChunk( FileHistory* fh, const QByteArray& ba, int start )
{
    RevMap& r = fh->revs;
    int nextStart;
    Rev* rev;
    ...
    if( !isMainHistory( fh ) && !fh->renamedPatches.isEmpty() &&
        fh->renamedPatches.contains( sha ) )
    {
        ...
        Rev* c =
            fakeRevData( sha, rev->parents(), rev->author(), rev->authorDate(), rev->shortLog(),
                         rev->longLog(), fh->renamedPatches[ sha ], prevSha->orderIdx, fh );
    ...
    if( isStGIT )
    {
        ...
        else if( patchesStillToFind > 0 || !isMainHistory( fh ) )
        {
            //
...
bool
Git::copyDiffIndex( FileHistory* fh, SCRef parent )
{
    //
...
void
Git::setLane( SCRef sha, FileHistory* fh )
{
    Lanes* l = fh->lns;
...
void
Git::updateLanes( Rev& c, Lanes& lns, SCRef sha )
{
    //
...
void
Git::procReadyRead( const QByteArray& fileChunk )
{
...
void
Git::flushFileNames( FileNamesLoader& fl )
{
    if( !fl.rf )
        return;
...
void
Git::appendFileName( RevFile& rf, SCRef name, FileNamesLoader& fl )
{
    if( fl.rf != &rf )
    {
        flushFileNames( fl );
...
void
Git::updateDescMap( const Rev* r, uint idx, QHash< QPair< uint, uint >, bool >& dm,
...
void
Git::mergeBranches( Rev* p, const Rev* r )
{
    int r_descBrnMaster =
        ( checkRef( r->sha(), BRANCH | RMT_BRANCH ) ? r->orderIdx : r->descBrnMaster );
...
void
Git::mergeNearTags( bool down, Rev* p, const Rev* r, const QHash< QPair< uint, uint >, bool >& dm )
{
