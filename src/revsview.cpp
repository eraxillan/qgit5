...
RevsView::RevsView( MainImpl* mi, Git* g, bool isMain ) : Domain( mi, g, isMain )
{
    ...
    connect( tab()->listViewLog,
             SIGNAL( lanesContextMenuRequested( const QStringList&, const QStringList& ) ),
             this,
             SLOT( on_lanesContextMenuRequested( const QStringList&, const QStringList& ) ) );
...
void
RevsView::setTabLogDiffVisible( bool b )
{
    QStackedWidget* s = tab()->stackedPanes;
    QTabWidget* t = tab()->tabLogDiff;
...
void
RevsView::viewPatch( bool newTab )
{
    ...
    {  // linkedPatchView == NULL
        ...
        connect( m(), SIGNAL( highlightPatch( const QString&, bool ) ), pv->tab()->textEditDiff,
                 SLOT( on_highlightPatch( const QString&, bool ) ) );

        connect( pv->tab()->fileList, SIGNAL( itemDoubleClicked( QListWidgetItem* ) ), m(),
                 SLOT( fileList_itemDoubleClicked( QListWidgetItem* ) ) );
    }
...
void
RevsView::on_newRevsAdded( const FileHistory* fh, const QVector< ShaString >& )
{
...
void
RevsView::on_loadCompleted( const FileHistory* fh, const QString& stats )
{
...
RevsView::doUpdate( bool force )
{
    ...
    {
        const QString tmp( "Sorry, revision " + st.sha() + " has not been found in main view" );
    ...
    {  // sha could be NULL
        ...
        if( st.isChanged( StateInfo::SHA ) || force )
        {
            ...
            if( testFlag( QGit::MSG_ON_NEW_F ) && tab()->textEditDiff->isVisible() )
                toggleDiffView();
        }
...
void
RevsView::updateLineEditSHA( bool clear )
{
...
void
RevsView::on_lanesContextMenuRequested( SCList parents, SCList children )
{
    QMenu contextMenu;
