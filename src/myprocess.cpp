...
bool
MyProcess::runAsync( SCRef rc, QObject* rcv, SCRef buf )
{
    async = true;
    runCmd = rc;
    receiver = rcv;
...
bool
MyProcess::runSync( SCRef rc, QByteArray* ro, QObject* rcv, SCRef buf )
{
    async = false;
    runCmd = rc;
    runOutput = ro;
...
MyProcess::setupSignals()
{
    ...
    if( receiver )
    {
        ...
        connect( this, SIGNAL( procDataReady( const QByteArray& ) ), receiver,
...
void
MyProcess::sendErrorMsg( bool notStarted, SCRef err )
{
...
bool
MyProcess::launchMe( SCRef runCmd, SCRef buf )
{
...
void
MyProcess::on_readyReadStandardOutput()
{
    if( canceling )
        return;
...
void
MyProcess::on_readyReadStandardError()
{
    if( canceling )
        return;
...
void
MyProcess::on_finished( int exitCode, QProcess::ExitStatus exitStatus )
{
    //
    ...
    isErrorExit = ( exitStatus != QProcess::NormalExit ) || ( exitCode != 0 && isWinShell ) ||
                  !errorDesc.isEmpty() || canceling;
...
MyProcess::splitArgList( SCRef cmd )
{
    ...
    //
    if( !( cmd.contains( QGit::QUOTE_CHAR ) || cmd.contains( "\"" ) || cmd.contains( "\'" ) ) )
        return cmd.split( ' ', QString::SkipEmptyParts );

    //
    // We have some work to do: first find a possible separator
    //
...
void
MyProcess::restoreSpaces( QString& newCmd, const QChar& sepChar )
{
    //
    ...
    for( int i = 0; i < newCmd.length(); i++ )
    {
        ...
        if( !replace && ( c == QGit::QUOTE_CHAR[ 0 ] || c == '\"' || c == '\'' ) &&
            ( newCmd.count( c ) % 2 == 0 ) )
        {
            replace = true;
            quoteChar = c;
