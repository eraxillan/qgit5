...
public:
    ...
    virtual void highlightBlock( const QString& p )
    {
        ...
        if( f->isRangeFilterActive && f->rangeInfo->start != 0 &&
            f->rangeInfo->start <= currentBlockState() && f->rangeInfo->end >= currentBlockState() )
        {
...
void
FileContent::setup( Domain* dm, Git* g, QListWidget* lw )
{
    d = dm;
    git = g;
    st = &( d->st );
    ...
    connect( d->m(), SIGNAL( typeWriterFontChanged() ), this, SLOT( typeWriterFontChanged() ) );

    connect( git, SIGNAL( annotateReady( Annotate*, bool, const QString& ) ), this,
             SLOT( on_annotateReady( Annotate*, bool, const QString& ) ) );

    connect( listWidgetAnn, SIGNAL( itemDoubleClicked( QListWidgetItem* ) ), this,
             SLOT( on_list_doubleClicked( QListWidgetItem* ) ) );

    QScrollBar* vsb = verticalScrollBar();
    connect( vsb, SIGNAL( valueChanged( int ) ), this, SLOT( on_scrollBar_valueChanged( int ) ) );
    vsb->setSingleStep( fontMetrics().lineSpacing() );

    vsb = listWidgetAnn->verticalScrollBar();
    connect( vsb, SIGNAL( valueChanged( int ) ), this,
             SLOT( on_listScrollBar_valueChanged( int ) ) );
    vsb->setSingleStep( fontMetrics().lineSpacing() );
}

void
FileContent::on_scrollBar_valueChanged( int value )
{
    listWidgetAnn->verticalScrollBar()->setValue( value );
}

void
FileContent::on_listScrollBar_valueChanged( int value )
{
    verticalScrollBar()->setValue( value );
}

int
FileContent::itemAnnId( QListWidgetItem* item )
{
    if( item == NULL )
        return 0;
...
bool
FileContent::isFileAvailable() const
{
    return isFileAvail;
}

bool
FileContent::isAnnotateAvailable() const
{
    return ( curAnn != NULL );
}

void
FileContent::on_list_doubleClicked( QListWidgetItem* item )
{
...
void
FileContent::clearAnnotate( bool emitSignal )
{
...
void
FileContent::clearText( bool emitSignal )
{
...
void
FileContent::clearAll( bool emitSignal )
{
...
void
FileContent::setShowAnnotate( bool b )
{
    //
    ...
    if( !isFileAvail || ( !curAnn && isShowAnnotate ) ||
...
void
FileContent::setHighlightSource( bool b )
{
...
void
FileContent::doUpdate( bool force )
{
...
bool
FileContent::startAnnotate( FileHistory* fh, SCRef ht )
{
    if( !isImageFile )
    {
...
uint
FileContent::annotateLength( const FileAnnotation* annFile )
{
    int maxLen = 0;
...
bool
FileContent::getRange( SCRef sha, RangeInfo* r )
{
    if( annotateObj )
    {
...
void
FileContent::scrollLineToTop( int lineNum )
{
...
int
FileContent::positionToLineNum( int pos )
{
    QTextCursor tc = textCursor();
    if( pos != -1 )
        tc.setPosition( pos );
...
int
FileContent::lineAtTop()
{
    return cursorForPosition( QPoint( 1, 1 ) ).blockNumber();
}

void
FileContent::setSelection( int paraFrom, int indexFrom, int paraTo, int indexTo )
{
...
void
FileContent::restoreScreenState()
{
    if( !ss.isValid )
        return;
...
void
FileContent::goToAnnotation( int revId, int dir )
{
...
bool
FileContent::goToRangeStart()
{
    if( !isRangeFilterActive || !curAnn || ( rangeInfo->start == 0 ) )
        return false;
...
void
FileContent::copySelection()
{
    QClipboard* cb = QApplication::clipboard();
    QString sel( textCursor().selectedText() );
    //
    //  FIXME: Workaround a Qt issue, QTextCursor::selectedText()
...
bool
FileContent::rangeFilter( bool b )
{
    ...
    if( b )
    {
        ...
        try
        {
            ...
            //
            // NOTE: could call qApp->processEvents()
            //
    ...
    {
        //
        // Bold if the first line was highlighted
        //
...
bool
FileContent::lookupAnnotation()
{
    if( st->sha().isEmpty() || st->fileName().isEmpty() || isAnnotationLoading || !annotateObj )
        return false;
...
void
FileContent::on_annotateReady( Annotate* readyAnn, bool ok, const QString& msg )
{
    //
    // Git::annotateReady() is sent to all receivers
    //
...
void
FileContent::typeWriterFontChanged()
{
    setFont( QGit::TYPE_WRITER_FONT );
...
void
FileContent::procReadyRead( const QByteArray& fileChunk )
{
    fileRowData.append( fileChunk );
    //
    // Set text at the end of loading, it is much faster
    //
}

void
FileContent::procFinished( bool emitSignal )
{
    if( isImageFile )
    {
        showFileImage();
...
void
FileContent::showFileImage()
{
    QTemporaryFile f;
    if( f.open() )
    {
        QString header( "<p class=Image><img src=\"" + f.fileName() + "\"></p>" );
...
FileContent::setAnnList()
{
    ...
    //
    // QTextEdit adds a blank line after content
    //
    sl.append( QString() );
    ...
    //
    listWidgetAnn->adjustSize();
    adjustAnnListSize( width );
    listWidgetAnn->setUpdatesEnabled( true );
}

void
FileContent::adjustAnnListSize( int width )
{
    ...
    //
    // Move textedit view to the left of listWidgetAnn
    //
    setViewportMargins( width, 0, 0, 0 );
}

void
FileContent::resizeEvent( QResizeEvent* e )
{
