...
{
    //
    // Avoid a Qt warning in case we are destroyed while still running
    //
    waitForFinished( 1000 );
}

void
DataLoader::on_cancel( const FileHistory* f )
{
    if( f == fh )
        on_cancel();
}
...
bool
DataLoader::start( SCList args, SCRef wd, SCRef buf )
{
    if( !isProcExited )
    {
...
void
DataLoader::on_finished( int, QProcess::ExitStatus )
{
    isProcExited = true;
...
void
DataLoader::on_timeout()
{
    if( canceling )
    {
        //
        // We leave with guiUpdateTimer not active
        //
        deleteLater();
        return;
    }
...
void
DataLoader::parseSingleBuffer( const QByteArray& ba )
{
...
void
DataLoader::addSplittedChunks( const QByteArray* hc )
{
...
void
DataLoader::baAppend( QByteArray** baPtr, const char* ascii, int len )
{
    if( *baPtr )
    {
        //
        // NOTE: we cannot use QByteArray::append(const char*)
...
bool
DataLoader::createTemporaryFile()
{
    return true;
}
...
DataLoader::createTemporaryFile()
{
    ...
    if( foundTmpDir && dir.absolutePath() != QDir::tempPath() )
    {
        ...
        if( !dataFile->open() )
        {
            ...
            dataFile = new UnbufferedTemporaryFile( this );
            dbs(
                "WARNING: directory '/tmp' is not writable, "
