...
void
SettingsImpl::addConfigOption( QTreeWidgetItem* parent, QStringList paths, const QString& value )
{
    if( paths.isEmpty() )
    {
...
void
SettingsImpl::readGitConfig( const QString& source )
{
...
void
SettingsImpl::treeWidgetGitConfig_itemChanged( QTreeWidgetItem* item, int i )
{
...
void
SettingsImpl::comboBoxUserSrc_activated( int i )
{
...
void
SettingsImpl::comboBoxGitConfigSource_activated( int )
{
    readGitConfig( comboBoxGitConfigSource->currentText() );
}

void
SettingsImpl::writeSetting( const QString& key, const QVariant& value )
{
    QSettings settings;
...
void
SettingsImpl::setupCodecList( QStringList& list )
{
    int i = 0;
    while( en[ i ] != 0 )
...
void
SettingsImpl::comboBoxCodecs_activated( int idx )
{
...
void
SettingsImpl::pushButtonExtDiff_clicked()
{
    QString extDiffName( QFileDialog::getOpenFileName( this, "Select the patch viewer" ) );
    if( !extDiffName.isEmpty() )
        lineEditExternalDiffViewer->setText( extDiffName );
}

void
SettingsImpl::pushButtonFont_clicked()
{
    bool ok;
...
void
SettingsImpl::changeFlag( uint f, bool b )
{
    setFlag( f, b );
    emit flagChanged( f );
}
...
void
SettingsImpl::checkBoxDiffCache_toggled( bool b )
{
...
void
SettingsImpl::checkBoxNumbers_toggled( bool b )
{
    changeFlag( NUMBERS_F, b );
}

void
SettingsImpl::checkBoxSign_toggled( bool b )
{
    changeFlag( SIGN_PATCH_F, b );
}

void
SettingsImpl::checkBoxRangeSelectDialog_toggled( bool b )
{
    changeFlag( RANGE_SELECT_F, b );
}

void
SettingsImpl::checkBoxReopenLastRepo_toggled( bool b )
{
    changeFlag( REOPEN_REPO_F, b );
}

void
SettingsImpl::checkBoxRelativeDate_toggled( bool b )
{
    changeFlag( REL_DATE_F, b );
}

void
SettingsImpl::checkBoxLogDiffTab_toggled( bool b )
{
    changeFlag( LOG_DIFF_TAB_F, b );
}

void
SettingsImpl::checkBoxSmartLabels_toggled( bool b )
{
    changeFlag( SMART_LBL_F, b );
}

void
SettingsImpl::checkBoxMsgOnNewSHA_toggled( bool b )
{
    changeFlag( MSG_ON_NEW_F, b );
}

void
SettingsImpl::checkBoxCommitSign_toggled( bool b )
{
    changeFlag( SIGN_CMT_F, b );
}

void
SettingsImpl::checkBoxCommitVerify_toggled( bool b )
{
    changeFlag( VERIFY_CMT_F, b );
}

void
SettingsImpl::checkBoxCommitUseDefMsg_toggled( bool b )
{
    changeFlag( USE_CMT_MSG_F, b );
}

void
SettingsImpl::lineEditExternalDiffViewer_textChanged( const QString& s )
{
    writeSetting( EXT_DIFF_KEY, s );
}

void
SettingsImpl::lineEditApplyPatchExtraOptions_textChanged( const QString& s )
{
    writeSetting( AM_P_OPT_KEY, s );
}

void
SettingsImpl::lineEditFormatPatchExtraOptions_textChanged( const QString& s )
{
    writeSetting( FMT_P_OPT_KEY, s );
}

void
SettingsImpl::lineEditExcludeFile_textChanged( const QString& s )
{
    writeSetting( EX_KEY, s );
}

void
SettingsImpl::lineEditExcludePerDir_textChanged( const QString& s )
{
    writeSetting( EX_PER_DIR_KEY, s );
}

void
SettingsImpl::lineEditTemplate_textChanged( const QString& s )
{
    writeSetting( CMT_TEMPL_KEY, s );
}

void
SettingsImpl::lineEditCommitExtraOptions_textChanged( const QString& s )
{
