...
void
FileList::setup( Domain* dm, Git* g )
{
    d = dm;
    git = g;
    st = &( d->st );
...
void
FileList::addItem( const QString& label, const QColor& clr )
{
...
void
FileList::on_changeFont( const QFont& f )
{
    setFont( f );
}

void
FileList::focusInEvent( QFocusEvent* )
{
    //
    // NOTE: Workaround a Qt 4.2 bug
    //
...
void
FileList::on_currentItemChanged( QListWidgetItem* current, QListWidgetItem* )
{
    if( !current )
        return;

    if( st->isMerge() && row( current ) == 0 )
    {
        //
        // Header clicked
        //
...
void
FileList::on_customContextMenuRequested( const QPoint& )
{
    int row = currentRow();
    //
    // Header clicked
    //
    if( row == -1 || ( row == 0 && st->isMerge() ) )
        return;
...
void
FileList::mousePressEvent( QMouseEvent* e )
{
...
void
FileList::mouseReleaseEvent( QMouseEvent* e )
{
    //
    // In case of just click without moving
    //
    d->setReadyToDrag( false );
    QListWidget::mouseReleaseEvent( e );
}

void
FileList::mouseMoveEvent( QMouseEvent* e )
{
    if( d->isReadyToDrag() )
    {
...
void
FileList::insertFiles( const RevFile* files )
{
    clear();
    if( !files )
        return;
    ...
    if( st->isMerge() )
    {
        const QString header( ( st->allMergeFiles() ) ? "Click to view only interesting files"
    ...
    for( int i = 0; i < files->count(); ++i )
    {
        ...
        if( extSt.isEmpty() )
        {
            ...
            {
                clr = useDark ? Qt::darkGreen : Qt::green;
            }
        ...
        {
            clr = useDark ? Qt::darkBlue : QColor( Qt::blue ).lighter();
...
void
FileList::update( const RevFile* files, bool newFiles )
{
    ...
    if( !fileName.isEmpty() && ( fileName == st->fileName() ) )
    {
        ...
        // Just a refresh
        //
        currentItem()->setSelected( st->selectItem() );
        return;
    }
    clearSelection();
