...
MainImpl::MainImpl( SCRef cd, QWidget* p ) : QMainWindow( p )
{
    ...
    connect( lineEditFilter, SIGNAL( returnPressed() ), this,
             SLOT( lineEditFilter_returnPressed() ) );
    ...
    //
    connect( File, SIGNAL( triggered( QAction* ) ), this,
             SLOT( openRecent_triggered( QAction* ) ) );
    ...
    connect( Actions, SIGNAL( triggered( QAction* ) ), this,
             SLOT( customAction_triggered( QAction* ) ) );
    ...
    connect( this, SIGNAL( changeFont( const QFont& ) ),
             git, SIGNAL( changeFont( const QFont& ) ) );
    ...
    QDir checkRepo;
    if( recents.size() >= 1 && testFlag( REOPEN_REPO_F, FLAGS_KEY ) &&
    ...
    //
    // MainImpl constructor is called before to enter event loop,
...
void
MainImpl::initWithEventLoopActive()
{
...
void
MainImpl::highlightAbbrevSha( SCRef abbrevSha )
{
    //
...
void
MainImpl::lineEditSHA_returnPressed()
{
...
void
MainImpl::ActExternalDiff_activated()
{
    QStringList args;
...
void
MainImpl::getExternalDiffArgs( QStringList* args, QStringList* filenames )
{
    //
    // Save files to diff in working directory,
    // they will be removed by ExternalDiffProc on exit
    //
    ...
    //
    // If the command doesn't have %1 and %2 to denote filenames, add them to end
    //
...
void
MainImpl::setRepository( SCRef newDir, bool refresh, bool keepSelection,
                         const QStringList* passedArgs, bool overwriteArgs )
{
    ...
    try
    {
        ...
        if( ActFilterTree->isChecked() )
            setWindowTitle( windowTitle() + " - FILTER ON < " + passedArgs->join( " " ) + " >" );
...
void
MainImpl::updateGlobalActions( bool b )
{
...
void
MainImpl::updateContextActions( SCRef newRevSha, SCRef newFileName, bool isDir, bool found )
{
...
void
MainImpl::listViewLog_doubleClicked( const QModelIndex& index )
{
...
void
MainImpl::histListView_doubleClicked( const QModelIndex& index )
{
...
void
MainImpl::fileList_itemDoubleClicked( QListWidgetItem* item )
{
...
void
MainImpl::treeView_doubleClicked( QTreeWidgetItem* item, int )
{
...
void
MainImpl::pushButtonCloseTab_clicked()
{
    Domain* t;
...
void
MainImpl::ActRangeDlg_activated()
{
    QString args;
...
void
MainImpl::ActViewFile_activated()
{
    openFileTab( firstTab< FileView >() );
}

void
MainImpl::ActViewFileNewTab_activated()
{
    openFileTab();
}

void
MainImpl::openFileTab( FileView* fv )
{
    if( !fv )
    {
        ...
        connect( fv->tab()->histListView, SIGNAL( doubleClicked( const QModelIndex& ) ), this,
                 SLOT( histListView_doubleClicked( const QModelIndex& ) ) );
...
void
MainImpl::ActViewDiff_activated()
{
    Domain* t;
    if( currentTabType( &t ) == TAB_FILE )
    {
...
void
MainImpl::ActViewDiffNewTab_activated()
{
    rv->viewPatch( true );
}

bool
MainImpl::eventFilter( QObject* obj, QEvent* ev )
{
...
void
MainImpl::revisionsDragged( SCList selRevs )
{
...
void
MainImpl::revisionsDropped( SCList remoteRevs )
{
    //
    ...
    do
    {
        ...
        {
            qDebug(
                "ASSERT in on_droppedRevisions: found %i files "
                "in %s",
                dr.count(), QGit::PATCHES_DIR.toLatin1().constData() );
            break;
        }
...
void
MainImpl::newRevsAdded( const FileHistory* fh, const QVector< ShaString >& )
{
    ...
    //
    if( !ActCommit->isEnabled() && ( !git->isNothingToCommit() || git->isUnknownFiles() ) &&
        !git->isCommittingMerge() )
        ActCommit_setEnabled( true );
}

void
MainImpl::lineEditFilter_returnPressed()
{
    ActSearchAndFilter->setChecked( true );
}

void
MainImpl::ActSearchAndFilter_toggled( bool isOn )
{
...
void
MainImpl::ActSearchAndHighlight_toggled( bool isOn )
{
...
void
MainImpl::filterList( bool isOn, bool onlyHighlight )
{
    ...
    if( isOn && !onlyHighlight )
        msg = QString(
                  "Found %1 matches. Toggle filter/highlight "
...
int
MainImpl::currentTabType( Domain** t )
{
    *t = NULL;
...
template < class X >
QList< X* >*
MainImpl::getTabs( QWidget* tabPage )
{
...
void
MainImpl::tabWdg_currentChanged( int w )
{
    if( w == -1 )
        return;
...
void
MainImpl::scrollTextEdit( int delta )
{
...
void
MainImpl::adjustFontSize( int delta )
{
    //
...
void
MainImpl::fileNamesLoad( int status, int value )
{
    switch( status )
    {
    case 1:  // Stop
...
void
MainImpl::updateCommitMenu( bool isStGITStack )
{
...
void
MainImpl::updateRecentRepoMenu( SCRef newEntry )
{
    //
...
void
MainImpl::doUpdateRecentRepoMenu( SCRef newEntry )
{
...
static int
cntMenuEntries( const QMenu& menu )
{
    int cnt = 0;
...
void
MainImpl::doContexPopup( SCRef sha )
{
...
void
MainImpl::doFileContexPopup( SCRef fileName, int type )
{
    QMenu contextMenu( this );
...
void
MainImpl::goRef_triggered( QAction* act )
{
    if( !act || act->data() != "Ref" )
        return;
...
void
MainImpl::ActSplitView_activated()
{
    Domain* t;
    switch( currentTabType( &t ) )
    {
...
void
MainImpl::ActToggleLogsDiff_activated()
{
    Domain* t;
...
void
MainImpl::ActShowDescHeader_activated()
{
    //
    // Each open tab get his description; could be different for each tab
    //
    emit updateRevDesc();
}

void
MainImpl::ActShowTree_toggled( bool b )
{
    if( b )
    {
        treeView->show();
...
void
MainImpl::ActSaveFile_activated()
{
    QFileInfo f( rv->st.fileName() );
...
void
MainImpl::openRecent_triggered( QAction* act )
{
    bool ok;
...
void
MainImpl::ActOpenRepo_activated()
{
    const QString dirName(
        QFileDialog::getExistingDirectory( this, "Choose a directory", curDir ) );
    if( !dirName.isEmpty() )
    {
        QDir d( dirName );
...
void
MainImpl::ActOpenRepoNewWindow_activated()
{
    const QString dirName(
        QFileDialog::getExistingDirectory( this, "Choose a directory", curDir ) );
    if( !dirName.isEmpty() )
...
void
MainImpl::refreshRepo( bool b )
{
    setRepository( curDir, true, b );
}

void
MainImpl::ActRefresh_activated()
{
    refreshRepo( true );
}

void
MainImpl::ActMailFormatPatch_activated()
{
    ...
    QString dirPath( QFileDialog::getExistingDirectory(
        this, "Choose destination directory - Save Patch", outDir ) );
    if( dirPath.isEmpty() )
        return;
...
bool
MainImpl::askApplyPatchParameters( bool* workDirOnly, bool* fold )
{
    int ret = 0;
    if( !git->isStGITStack() )
    {
        ret = QMessageBox::question( this, "Apply Patch",
                                     ...
                                     "working directory?",
                                     "&Cancel", "&Working directory", "&Commit", 0, 0 );
        *workDirOnly = ( ret == 1 );
        *fold = false;
    }
    else
...
void
MainImpl::ActMailApplyPatch_activated()
{
    QSettings settings;
    QString outDir( settings.value( PATCH_DIR_KEY, curDir ).toString() );
    QString patchName(
        QFileDialog::getOpenFileName( this, "Choose the patch file - Apply Patch", outDir,
...
void
MainImpl::ActCheckWorkDir_toggled( bool b )
{
...
void
MainImpl::ActSettings_activated()
{
    SettingsImpl setView( this, git );
    connect( &setView, SIGNAL( typeWriterFontChanged() ), this, SIGNAL( typeWriterFontChanged() ) );

    connect( &setView, SIGNAL( flagChanged( uint ) ), this, SIGNAL( flagChanged( uint ) ) );
...
void
MainImpl::ActCustomActionSetup_activated()
{
    CustomActionImpl* ca = new CustomActionImpl();  // NOTE: has Qt::WA_DeleteOnClose

    connect( this, SIGNAL( closeAllWindows() ), ca, SLOT( close() ) );
    connect( ca, SIGNAL( listChanged( const QStringList& ) ), this,
             SLOT( customActionListChanged( const QStringList& ) ) );

    ca->show();
}

void
MainImpl::customActionListChanged( const QStringList& list )
{
    //
    // Update menu of all windows
    //
...
void
MainImpl::doUpdateCustomActionMenu( const QStringList& list )
{
...
void
MainImpl::customAction_triggered( QAction* act )
{
    ...
    {
        bool ok;
        cmdArgs = QInputDialog::getText( this, "Run action - QGit",
                                         "Enter command line "
                                         "arguments for '" +
                                             actionName + "'",
                                         QLineEdit::Normal, "", &ok );
    ...
    ConsoleImpl* c = new ConsoleImpl( actionName, git );  // NOTE: has Qt::WA_DeleteOnClose attribute

    connect( this, SIGNAL( typeWriterFontChanged() ), c, SLOT( typeWriterFontChanged() ) );

    connect( this, SIGNAL( closeAllWindows() ), c, SLOT( close() ) );
    connect( c, SIGNAL( customAction_exited( const QString& ) ), this,
             SLOT( customAction_exited( const QString& ) ) );
...
void
MainImpl::customAction_exited( const QString& name )
{
...
void
MainImpl::ActCommit_activated()
{
    CommitImpl* c = new CommitImpl( git, false );  // NOTE: has Qt::WA_DeleteOnClose attribute
    connect( this, SIGNAL( closeAllWindows() ), c, SLOT( close() ) );
...
void
MainImpl::ActAmend_activated()
{
    CommitImpl* c = new CommitImpl( git, true );  // NOTE: has Qt::WA_DeleteOnClose attribute
    connect( this, SIGNAL( closeAllWindows() ), c, SLOT( close() ) );
    connect( c, SIGNAL( changesCommitted( bool ) ), this, SLOT( changesCommitted( bool ) ) );
    c->show();
}
...
void
MainImpl::changesCommitted( bool ok )
{
    if( ok )
        refreshRepo( false );
    else
...
void
MainImpl::ActCommit_setEnabled( bool b )
{
    //
    // NOTE: pop and push commands fail if there are local changes,
...
void
MainImpl::ActBranch_activated()
{
    doBranchOrTag( false );
}

void
MainImpl::ActTag_activated()
{
    doBranchOrTag( true );
}

void
MainImpl::doBranchOrTag( bool isTag )
{
    QString refDesc = isTag ? "tag" : "branch";
    QString boxDesc = "Make " + refDesc + " - QGit";
    QString revDesc( rv->tab()->listViewLog->currentText( LOG_COL ) );
    bool ok;
    QString ref = QInputDialog::getText( this, boxDesc, "Enter " + refDesc + " name:",
                                         QLineEdit::Normal, "", &ok );
    if( !ok || ref.isEmpty() )
        return;
    ...
    {
        QMessageBox::warning( this, boxDesc, "Sorry, " + refDesc +
                                                 " name already exists.\n"
    ...
    if( isTag )
    {
        msg = QInputDialog::getText( this, boxDesc, "Enter tag message, if any:", QLineEdit::Normal,
                                     revDesc, &ok );
        if( !ok )
            return;
    }
...
void
MainImpl::ActTagDelete_activated()
{
    if( QMessageBox::question( this, "Delete tag - QGit",
                               "Do you want to un-tag selected revision?", "&Yes", "&No", QString(),
                               0, 1 ) == 1 )
        return;
...
MainImpl::ActPush_activated()
{
    ...
    {
        const QString tmp(
            QString( "Pushing patch %1 of %2" ).arg( i + 1 ).arg( selectedItems.count() ) );
        statusBar()->showMessage( tmp );
...
void
MainImpl::ActFilterTree_toggled( bool b )
{
    if( !ActFilterTree->isEnabled() )
    {
...
void
MainImpl::ActFindNext_activated()
{
    QTextEdit* te = getCurrentTextEdit();
    ...
    while( true )
    {
        ...
        if( endOfDocument )
        {
            QMessageBox::warning( this, "Find text - QGit",
                                  "Text \"" + textToFind + "\" not found!", QMessageBox::Ok, 0 );
            return;
        }
        if( QMessageBox::question( this, "Find text - QGit",
                                   "End of document "
...
void
MainImpl::ActAbout_activated()
{
    static const char* aboutMsg =
        "<p><b>QGit version " PACKAGE_VERSION
        "</b></p>"
...
void
MainImpl::closeEvent( QCloseEvent* ce )
{
    saveCurrentGeometry();
...
void
MainImpl::ActClose_activated()
{
    close();
}

void
MainImpl::ActExit_activated()
{
