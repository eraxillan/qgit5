...
ListView::ListView( QWidget* parent )
    : QTreeView( parent ), d( NULL ), git( NULL ), fh( NULL ), lp( NULL )
{
}

void
ListView::setup( Domain* dm, Git* g )
{
    d = dm;
    git = g;
    fh = d->model();
...
ListView::~ListView()
{
    git->cancelDataLoading( fh );  // NOTE: non blocking
}

const QString
ListView::sha( int row ) const
{
...
void
ListView::scrollToNextHighlighted( int direction )
{
    //
...
void
ListView::scrollToCurrent( ScrollHint hint )
{
...
void
ListView::on_changeFont( const QFont& f )
{
    setFont( f );
...
int
ListView::getLaneType( SCRef sha, int pos ) const
{
...
void
ListView::getSelectedItems( QStringList& selectedItems )
{
    selectedItems.clear();
...
int
ListView::filterRows( bool isOn, bool highlight, SCRef filter, int colNum, ShaSet* set )
{
...
void
ListView::currentChanged( const QModelIndex& index, const QModelIndex& )
{
...
bool
ListView::filterRightButtonPressed( QMouseEvent* e )
{
...
void
ListView::mousePressEvent( QMouseEvent* e )
{
...
void
ListView::mouseReleaseEvent( QMouseEvent* e )
{
...
void
ListView::mouseMoveEvent( QMouseEvent* e )
{
    if( d->isReadyToDrag() )
    {
...
void
ListView::dragEnterEvent( QDragEnterEvent* e )
{
...
void
ListView::dragMoveEvent( QDragMoveEvent* e )
{
    //
...
void
ListView::dropEvent( QDropEvent* e )
{
...
void
ListView::on_customContextMenuRequested( const QPoint& pos )
{
...
bool
ListView::getLaneParentsChildren( SCRef sha, int x, SList p, SList c )
{
    ListViewDelegate* lvd = static_cast< ListViewDelegate* >( itemDelegate() );
...
QSize
ListViewDelegate::sizeHint( const QStyleOptionViewItem&, const QModelIndex& ) const
{
    return QSize( laneWidth(), laneHeight );
}

void
ListViewDelegate::diffTargetChanged( int row )
{
...
const Rev*
ListViewDelegate::revLookup( int row, FileHistory** fhPtr ) const
{
...
static QColor
blend( const QColor& col1, const QColor& col2, int amount = 128 )
{
...
void
ListViewDelegate::paintGraphLane( QPainter* p, int type, int x1, int x2, const QColor& col,
...
void
ListViewDelegate::paintGraph( QPainter* p, const QStyleOptionViewItem& opt,
                              const QModelIndex& i ) const
{
    static const QColor& baseColor = QPalette().color( QPalette::WindowText );
    static const QColor colors[ COLORS_NUM ] = {baseColor,    Qt::red, DARK_GREEN,  Qt::blue,
...
void
ListViewDelegate::paintLog( QPainter* p, const QStyleOptionViewItem& opt,
                            const QModelIndex& index ) const
{
    ...
    if( pm )
    {
        p->drawPixmap( newOpt.rect.x(), newOpt.rect.y() + 1,
                       *pm );  // +1 means leave a pixel spacing above the pixmap
...
void
ListViewDelegate::paint( QPainter* p, const QStyleOptionViewItem& opt,
                         const QModelIndex& index ) const
{
...
bool
ListViewDelegate::changedFiles( SCRef sha ) const
{
...
QPixmap*
ListViewDelegate::getTagMarks( SCRef sha, const QStyleOptionViewItem& opt ) const
{
...
void
ListViewDelegate::addRefPixmap( QPixmap** pp, SCRef sha, int type, QStyleOptionViewItem opt ) const
{
    QString curBranch;
...
void
ListViewDelegate::addTextPixmap( QPixmap** pp, SCRef txt, const QStyleOptionViewItem& opt ) const
{
    QPixmap* pm = *pp;
...
bool
ListViewProxy::isMatch( SCRef sha ) const
{
...
bool
ListViewProxy::isMatch( int source_row ) const
{
...
bool
ListViewProxy::isHighlighted( int row ) const
{
    //
...
int
ListViewProxy::setFilter( bool isOn, bool h, SCRef fl, int cn, ShaSet* s )
{
