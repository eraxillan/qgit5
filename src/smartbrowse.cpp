...
{
    this->setStyleSheet(
        "SmartLabel { border: 1px solid LightGray;"
        "padding: 0px 2px 2px 2px; }" );
}

void
SmartLabel::paintEvent( QPaintEvent* event )
{
...
void
SmartLabel::contextMenuEvent( QContextMenuEvent* e )
{
...
SmartBrowse::SmartBrowse( RevsView* par ) : QObject( par )
{
    ...
    connect( vsbLog, SIGNAL( valueChanged( int ) ), this, SLOT( updateVisibility() ) );

    connect( vsbDiff, SIGNAL( valueChanged( int ) ), this, SLOT( updateVisibility() ) );

    connect( logTopLbl, SIGNAL( linkActivated( const QString& ) ), this,
             SLOT( linkActivated( const QString& ) ) );

    connect( logBottomLbl, SIGNAL( linkActivated( const QString& ) ), this,
             SLOT( linkActivated( const QString& ) ) );

    connect( diffTopLbl, SIGNAL( linkActivated( const QString& ) ), this,
             SLOT( linkActivated( const QString& ) ) );

    connect( diffBottomLbl, SIGNAL( linkActivated( const QString& ) ), this,
             SLOT( linkActivated( const QString& ) ) );
}

void
SmartBrowse::setVisible( bool b )
{
    b = b && lablesEnabled;
...
QTextEdit*
SmartBrowse::curTextEdit( bool* isDiff )
{
    QTextEdit* log = static_cast< QTextEdit* >( rv->tab()->textBrowserDesc );
    QTextEdit* diff = static_cast< QTextEdit* >( rv->tab()->textEditDiff );
...
int
SmartBrowse::visibilityFlags( bool* isDiff )
{
    static int MIN = 5;
...
void
SmartBrowse::flagChanged( uint flag )
{
...
void
SmartBrowse::linkActivated( const QString& text )
{
    int key = text.toInt();
    switch( key )
    {
...
bool
SmartBrowse::eventFilter( QObject* obj, QEvent* event )
{
    if( !lablesEnabled )
...
bool
SmartBrowse::wheelRolled( int delta, int flags )
{
    ...
    bool scrollingOut =
        ( ( ( flags & AT_TOP ) && ( delta > 0 ) ) || ( ( flags & AT_BTM ) && ( delta < 0 ) ) );
