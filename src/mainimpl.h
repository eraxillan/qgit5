...
class MainImpl : public QMainWindow, public Ui_MainBase
{
    ...
    void setRepository( SCRef wd, bool = false, bool = false, const QStringList* = NULL,
                        bool = false );
    ...
    template < class X >
    QList< X* >* getTabs( QWidget* tabPage = NULL );
    template < class X >
    X* firstTab( QWidget* startPage = NULL );
...
public:
    ExternalDiffProc( const QStringList& f, QObject* p ) : QProcess( p ), filenames( f )
    {
