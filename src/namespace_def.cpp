...
static void
adjustPath( QStringList& args, bool* winShell )
{
    //
...
#elif defined( Q_OS_MACX )  // Mac OS X specific code
...
static void
adjustPath( QStringList& args, bool* )
{
    //
...
//
static inline uint
hexVal( const uchar* ch )
{
    return ( *ch < 64 ? *ch - 48 : *ch - 87 );
}

uint
qHash( const ShaString& s )
{
    //
    // Fast path, called 6-7 times per revision
    //
    const uchar* ch = reinterpret_cast< const uchar* >( s.latin1() );
    return ( hexVal( ch ) << 24 ) + ( hexVal( ch + 2 ) << 20 ) + ( hexVal( ch + 4 ) << 16 ) +
           ( hexVal( ch + 6 ) << 12 ) + ( hexVal( ch + 8 ) << 8 ) + ( hexVal( ch + 10 ) << 4 ) +
           hexVal( ch + 12 );
}
...
//
const ShaString
QGit::toTempSha( const QString& sha )
{
...
const ShaString
QGit::toPersistentSha( const QString& sha, QVector< QByteArray >& v )
{
...
//
// Default QFont constructor calls static method QApplication::font() that could
...
bool
QGit::testFlag( uint f, SCRef flagsVariable )
{
    return ( flags( flagsVariable ) & f );
}

void
QGit::setFlag( uint f, bool b, SCRef flagsVariable )
{
    QSettings settings;
...
//
void
QGit::saveGeometrySetting( SCRef name, QWidget* w, splitVect* svPtr )
{
    QSettings settings;
    if( w && w->isVisible() )
...
void
QGit::restoreGeometrySetting( SCRef name, QWidget* w, splitVect* svPtr )
{
    QSettings settings;
    QString nm;
    if( w )
...
//
bool
QGit::stripPartialParaghraps( const QByteArray& ba, QString* dst, QString* prev )
{
    if( ba.endsWith( '\n' ) )
    {
        //
...
bool
QGit::writeToFile( SCRef fileName, SCRef data, bool setExecutable )
{
    QFile file( fileName );
...
bool
QGit::writeToFile( SCRef fileName, const QByteArray& data, bool setExecutable )
{
    QFile file( fileName );
    if( !file.open( QIODevice::WriteOnly ) )
    {
...
bool
QGit::readFromFile( SCRef fileName, QString& data )
{
    data = "";
    QFile file( fileName );
...
bool
QGit::startProcess( QProcess* proc, SCList args, SCRef buf, bool* winShell )
{
