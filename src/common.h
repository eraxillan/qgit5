...
template <>
inline const QString
_valueOf( const QStringList& x )
{
    return x.join( " " );
}

inline const QString&
_valueOf( const QString& x )
{
    return x;
}
...
#define FOREACH( type, i, c )                                                                      \
    for( type::const_iterator i( ( c ).constBegin() ), _e##i##_( ( c ).constEnd() );               \
         i != _e##i##_; ++i )
...
isInfoCol( int x )
{
    return ( x == TIME_COL || x == LOG_COL || x == AUTH_COL );
}
...
const int FLAGS_DEF = USE_CMT_MSG_F | RANGE_SELECT_F | SMART_LBL_F | VERIFY_CMT_F | SIGN_PATCH_F |
                      LOG_DIFF_TAB_F | MSG_ON_NEW_F;
...
struct ShaString : public QLatin1String
{
    ...
    bool operator==( const ShaString& o ) const
    {
...
public:
    ...
    int dirAt( uint idx ) const;
    int nameAt( uint idx ) const;
...
struct FileAnnotation
{
    ...
    explicit FileAnnotation( int id );
    FileAnnotation();
};
...
{
public:
    DeferredPopupEvent( SCRef msg, int type );
};
...
public:
    MainExecErrorEvent( SCRef c, SCRef e );
