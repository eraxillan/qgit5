...
FileHistory::FileHistory( QObject* p, Git* g ) : QAbstractItemModel( p ), git( g )
{
    ...
    connect( git, SIGNAL( changeFont( const QFont& ) ),
...
void
FileHistory::resetFileNames( SCRef fn )
{
    fNames.clear();
    fNames.append( fn );
    curFNames = fNames;
...
void
FileHistory::setEarlyOutputState( bool b )
{
    earlyOutputCnt = ( b ? earlyOutputCntBase : -1 );
}

void
FileHistory::setAnnIdValid( bool b )
{
    annIdValid = b;
}

int
FileHistory::rowCount( const QModelIndex& parent ) const
{
    return ( !parent.isValid() ? rowCnt : 0 );
}

bool
FileHistory::hasChildren( const QModelIndex& parent ) const
{
    return !parent.isValid();
}

int
FileHistory::columnCount( const QModelIndex& ) const
{
    return 5;
}

int
FileHistory::row( SCRef sha ) const
{
...
const QStringList
FileHistory::fileNames() const
{
    return fNames;
}

const QString
FileHistory::sha( int row ) const
{
    return ( row < 0 || row >= rowCnt ? "" : QString( revOrder.at( row ) ) );
}

void
FileHistory::flushTail()
{
...
void
FileHistory::clear( bool complete )
{
#if QT_VERSION >= 0x050000
    beginResetModel();
#endif
...
void
FileHistory::on_newRevsAdded( const FileHistory* fh, const QVector< ShaString >& shaVec )
{
    // signal newRevsAdded() is broadcast
    if( fh != this )
        return;
...
void
FileHistory::on_loadCompleted( const FileHistory* fh, const QString& )
{
...
void
FileHistory::on_changeFont( const QFont& f )
{
...
Qt::ItemFlags
FileHistory::flags( const QModelIndex& ) const
{
    return Qt::ItemIsEnabled | Qt::ItemIsSelectable;  // read only
}

QVariant
FileHistory::headerData( int section, Qt::Orientation orientation, int role ) const
{
...
QModelIndex
FileHistory::parent( const QModelIndex& ) const
{
...
const QString
FileHistory::timeDiff( unsigned long secs ) const
{
...
QVariant
FileHistory::data( const QModelIndex& index, int role ) const
{
